...
알파 카드(2026-05-28) + AI 매매 일기(2026-05-28) 출시 완료. 다음 차별화 카드:

1. **Paper Portfolio + 백테스트** (spec §3 우선순위 2) — `paper_holdings`·`paper_transactions` 테이블 + 가상 자금 + 백테스트 엔진. 3축 가치(실/Paper/AI 학습)의 마지막 축
2. **AI 시스템 프롬프트 교육자 역할** (spec §3 차별화) — 채팅에서 "PER이 뭐야?" 같은 개념 질문에 친절히 답하도록 system prompt 한 줄 추가
