...
알파 카드 출시 완료(2026-05-28). 다음 차별화 카드:

1. **AI 매매 일기** (spec §3 우선순위 1, 한국 핀테크에 없는 기능) — `journal_entries` 테이블 + 매매당 이유 기록 + `analyze_journal` 도구 + UI
2. **Paper Portfolio + 백테스트** (spec §3 우선순위 2) — `paper_holdings`·`paper_transactions` 테이블 + 가상 자금 + 백테스트 엔진
...
| **Phase 2 핵심 차별화** | **Paper Portfolio + 백테스트** | 가상 자금으로 매매 시뮬레이션. 실 포트폴리오와 분리된 모드. 5년 일봉 데이터로 과거 시점 백테스트. 본 서비스의 3축 가치 중 하나(랜딩에 노출 중) |
| **Phase 2 핵심 차별화** | **AI 매매 일기** | 사용자가 매매 이유 기록 → Claude가 사후 패턴 분석("8월에 감정적 매도 패턴"). 실/Paper 양쪽 모두 적용. 규제 무관, 본 서비스의 가장 큰 차별화 카드 |
| Phase 2 차별화 | AI 분석가 = 학습 도우미 | 시스템 프롬프트에 "개념 질문(PER, 분산투자 등)에는 친절히 교육 답변" 추가. 별도 컨텐츠 X, AI에 위임 |
...
| Tier | 작업 |
|---|---|
| 1 | **Paper Portfolio 모드** — `paper_holdings`·`paper_transactions` 테이블 + UI 탭 분리 + 가상 자금 잔고 |
| 1 | **백테스트 엔진** — 과거 시점 + 전략(정액 적립, 리밸런싱) → 시뮬레이션 결과 비교 |
| 1 | **AI 매매 일기** — `journal_entries` 테이블 + 매매당 이유 기록 + Claude 사후 패턴 분석 도구 |
